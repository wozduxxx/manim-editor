 ...
 *     which is the safest way to reset leva state between objects
 */
...
const levaTheme = {
  colors: {
    elevation1: '#0d0d18',   // panel bg
    elevation2: '#13131f',   // input bg
    elevation3: '#1a1a28',   // hover bg
    accent1: '#58C4DD',
    accent2: '#7ad6e8',
    accent3: '#9ae2f0',
    highlight1: '#3a3a58',
...
// ── Inner panel — gets a fresh store per object via key=obj.id ─────────────────
...
function ObjectControls({ obj }: { obj: SceneObject }) {
  ...
  // Build geometry controls depending on type
      ...
      Position: folder(
        {
          posX: {
            ...
            label: 'X',
            onChange: (v: number) => update({ position: [v, obj.props.position[1], obj.props.position[2]] }),
            transient: false,
          },
          posY: {
            ...
            label: 'Y',
            onChange: (v: number) => update({ position: [obj.props.position[0], v, obj.props.position[2]] }),
            transient: false,
          },
        },
        { collapsed: false }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          color: '#3a3a55', textTransform: 'uppercase', fontFamily: 'monospace',
        }}>
          Properties
        </span>
        {obj && (
      ...
      {!obj ? (
        <div style={{
          ...
          gap: 8, color: '#1e1e30', fontFamily: 'monospace',
          fontSize: 11, textAlign: 'center', padding: 20,
        }}>
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
            <circle cx="14" cy="14" r="9" stroke="currentColor" strokeWidth="1.2" strokeDasharray="3 2.5" />
          </svg>
          Select an object<br />to edit properties
        </div>
      ) : (
        // key=obj.id forces a full React remount when selection changes,
        // which resets the leva store cleanly with the new object's values.
        ...
        <div style={{ padding: '8px 10px', borderTop: '1px solid #1e1e2e', flexShrink: 0 }}>
          <button
            ...
            style={{
              ...
              color: '#6a3333', letterSpacing: '0.06em',
