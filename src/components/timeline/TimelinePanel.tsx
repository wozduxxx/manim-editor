...
export default function TimelinePanel({ canvasRef }: Props) {
  ...
  // ← ЛОКАЛЬНЫЙ STATE вместо Redux scrubTime
  ...
  // ── DRAG PLAYHEAD — вызывает seekPreview на канвасе ────────────────────────
  const handleRulerMouseDown = useCallback((e: React.MouseEvent) => {
    if (!rulerRef.current || !scrollRef.current || !canvasRef.current) return;

    const rect = rulerRef.current.getBoundingClientRect();
    const scrollX = scrollRef.current.scrollLeft;
    const x = e.clientX - rect.left + scrollX;
    const time = Math.max(0, x / PX_PER_S);

    // ← Останавливаем playback и начинаем drag
    if (isPlaying) canvasRef.current.stopAll();
    setDragTime(Math.min(time, totalDur));

    // ← Вызываем seekPreview на канвасе
    canvasRef.current.seekPreview(Math.min(time, totalDur));

    const handleMouseMove = (moveEvent: MouseEvent) => {
      const moveX = moveEvent.clientX - rect.left + scrollX;
      const moveTime = Math.max(0, moveX / PX_PER_S);
      const clamped = Math.min(moveTime, totalDur);
      setDragTime(clamped);
      canvasRef.current?.seekPreview(clamped);
    };

    const handleMouseUp = () => {
      document.removeEventListener('mousemove', handleMouseMove);
      document.removeEventListener('mouseup', handleMouseUp);
      setDragTime(null); // ← Сбрасываем drag, но previewTime остаётся на канвасе
    };
  ...
  // Вычисляем текущее время для playhead
  const currentPlayheadTime = dragTime ?? (isPlaying
      ? (startOf.get(sorted[curStep]?.id) ?? 0)
      : 0);
  ...
  return (
      <div style={{
        ...
        <span style={{
          ...
          <button
              onClick={() => canvasRef.current?.stopAll()}
              disabled={!isPlaying}
              style={{
                width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center',
                borderRadius: 5, border: '1px solid #1e1e2e', background: 'none',
                color: !isPlaying ? '#1e1e2e' : '#4a4a70', cursor: !isPlaying ? 'default' : 'pointer',
          ...
          {isPlaying
              ? `step ${curStep + 1}/${sorted.length}`
              : `${sorted.length} steps · ${totalDur.toFixed(1)}s`}
        </span>
          ...
          {activeObjId && (
              <>
            <span style={{ fontSize: 10, color: '#3a3a55', fontFamily: 'monospace' }}>
              Add to:
            </span>
                <select
                    disabled={isPlaying}
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            {byObj.length === 0 && (
                <div style={{ padding: '10px 12px', fontSize: 10, color: '#222234', fontFamily: 'monospace' }}>
                  No objects
                </div>
            )}
            ...
            {byObj.map(({ obj }) => (
                ...
                >
                  ...
                  <span style={{
                    ...
                    color: obj.visible ? '#7070a0' : '#2a2a40',
          ...
          >
            <div style={{ width: trackW, position: 'relative' }}>
              ...
              >
                {Array.from({ length: Math.ceil(totalDur) + 3 }).map((_, i) => (
                    ...
                    >
                      ...
                      <span style={{
                        fontSize: 8, color: '#333355', fontFamily: 'monospace', paddingLeft: 3
                      }}>
                    {i}s
                  </span>
                    </div>
                ...
                {/* Playhead — показывает dragTime или currentTime */}
                <div style={{
                  position: 'absolute',
                  left: (currentPlayheadTime * PX_PER_S),
                  top: 0,
                  width: 2,
                  height: '100%',
                  background: '#83C167',
              ...
              {byObj.map(({ obj, steps: objSteps }) => (
                  <div
                      key={obj.id}
                      style={{ position: 'relative', height: ROW_H, borderBottom: '1px solid #10101c' }}
                  >
                    {Array.from({ length: Math.ceil(totalDur) + 3 }).map((_, i) => (
                        <div
                            key={i}
                            style={{
                              position: 'absolute', left: i * PX_PER_S, top: 0, width: 1, height: '100%',
                              background: '#13131e'
                            }}
                        />
                    ))}
                    ...
                    {objSteps.map(step => {
                      ...
                      <span style={{
                        ...
                        {step.animationType.slice(0, 7)}
                            ...
                            <span style={{ fontSize: 8, color: `${color}70`, marginLeft: 4, fontFamily: 'monospace' }}>
                        {step.duration}s
                      </span>
                            <button
                                onClick={e => {
                                ...
                                style={{
                                  marginLeft: 'auto', fontSize: 8, color: `${color}60`,
              ...
              <span style={{ color }}>{selStep.animationType}</span>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#4a4a70' }}>
                  ...
                  <input
                      ...
                      onChange={e => dispatch(updateAnimationStep({
                        id: selStep.id,
                      ...
                      <input
                          ...
                          onChange={e => {
                            const pos: [number, number, number] = [
                              ...(selStep.params?.targetPosition ?? [0, 0, 0])
